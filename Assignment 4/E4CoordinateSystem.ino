...
{      
  ...
  */
  ...
  display.drawLine(0,0,90, 0, WHITE);
  drawBackground();
...
void drawBackground(){
  for(int i = 0; i < SCREEN_HEIGHT; i++){
    display.drawLine(0, i, SCREEN_WIDTH, i, BLACK);
  }
...
//  display.drawCircle(int(xPos), int(yPos), int(circ_dia), BLACK);
...
//  xPos = (SCREEN_WIDTH / 2) + accel_angle_x;
//  yPos = (SCREEN_HEIGHT / 2) + accel_angle_y;

//  display.drawCircle(int(xPos), int(yPos), int(circ_dia), WHITE);
...
//  display.display();
  ...
  handlePhysics(accel_angle_x, accel_angle_y, accel_angle_z);
...
//  drawBackground();
  ...
  if(calibrationTimer > 0){
    display.println("Calibrating Device");
    display.display();
    return;
  }
  ...
  display.print("z: ");
  display.println(zPos);

//  display.print("xSpeed: ");
//  display.println(xSpeed);
//  display.print("Base Acceleration X: ");
//  display.println(baseAccelX);
//  display.print("Real Acceleration X: ");
//  display.println(currentAccelX);
//  display.print("True Acceleration X: ");
//  display.println(trueAccelX);

  display.display();
}

void handlePhysics(float accelX, float accelY, float accelZ){
  if(calibrationTimer > 0){
    calibrationTimer -= 25;
    baseAccelX = accelX;
    baseAccelY = accelY;
  ...
  trueAccelX = accelX - baseAccelX;
  trueAccelY = accelY - baseAccelY;
  trueAccelZ = accelZ - baseAccelZ;


  float offset = 5;

  if(abs(trueAccelX) < offset){
    trueAccelX = 0;
  }

  if(abs(trueAccelY) < offset){
    trueAccelY = 0;
  }

  if(abs(trueAccelZ) < offset){
    trueAccelZ = 0;
  }

  joltX = trueAccelBX - trueAccelX;
  joltY = trueAccelBY - trueAccelY;
  joltZ = trueAccelBZ - trueAccelZ;
...
//  CheckSpeed(offset);
  ...
  trueAccelBX = trueAccelX;
  trueAccelBY = trueAccelY;
  trueAccelBZ = trueAccelZ;
...
void CheckSpeed(float offset){
  if(abs(xSpeed) < offset){
    xSpeed = 0;
  }

  if(abs(ySpeed) < offset){
    ySpeed = 0;
  }

  if(abs(zSpeed) < offset){
    zSpeed = 0;
  }
}
