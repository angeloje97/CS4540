...
//  display.display();
  ...
  handlePhysics(accel_angle_x, accel_angle_y);
  handleDrawCircle();
  delay(25); 
...
void handleDrawCircle(){
  ...
  display.drawCircle(int(xPos), int(yPos), int(1), BLACK);
  display.display();

}

void handlePhysics(float accelX, float accelY){
  deltaXAccel = xAccel - accelX;
  deltaYAccel = yAccel - accelY;
  xAccel = accelX;
  yAccel = accelY;

  if(abs(deltaXAccel) > 30){
    OnShake();
  }

  if(abs(deltaYAccel) > 30){
    OnShake();
  }
